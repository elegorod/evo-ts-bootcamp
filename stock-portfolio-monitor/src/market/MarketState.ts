...
export default class MarketState {
  ...
  *load() {
    ...
    try {
      if (this.currentPrices.length === 0) {
        this.currentPrices = yield Promise.all(this.portfolioState.uniqueTickers.map(async ticker =>
          ({ ticker: ticker, price: await loadCurrentPrice(ticker) })))
      }
      let info = this.periodInfo.find(_ => _.period === this.period)
      if (!info) {
  ...
  private async loadCandleGroups(): Promise<TickerCandleGroup[]> {
    ...
    const initialDateFrom = this.period === ViewPeriod.Year ? dateFrom.minusMonths(1) : dateFrom.minusDays(MAX_CONSECUTIVE_DAYS_OFF)
    ...
    return Promise.all(this.portfolioState.uniqueTickers.map(async ticker => {
      ...
      const firstIndex = allCandles.findIndex(_ => _.instant.epochSecond() >= dateFromInstant.epochSecond())
      let candles
      if (firstIndex < 2) {
        candles = allCandles
      } else {
  ...
  private updateSelectedPeriodInfo(stockProfits: StockProfit[], candleGroups: TickerCandleGroup[], profitPoints: ProfitPoint[]) {
    let info = this.periodInfo.find(_ => _.period === this.period)
  ...
  private calculateStockProfits(candleGroups: TickerCandleGroup[]): StockProfit[] {
    return this.portfolioState.uniqueTickers.map(ticker => {
      ...
      if (firstCandle) {
        const currentPrice = this.currentPrices.find(_ => _.ticker === ticker)?.price
        if (currentPrice !== undefined) {
          const firstDate = firstCandle.instant.atZone(ZoneOffset.UTC).toLocalDate()
          const items = this.portfolioState.items.filter(_ => _.ticker === ticker)
          items.forEach(item => {
      ...
      return {
        ...
        value: value,
        percent: percent,
        previousInvestedValue: previousInvestedValue
      }
    })
  }

  private findFirstCandle(ticker: Ticker, candleGroups: TickerCandleGroup[]): Candle | undefined {
    return candleGroups.find(_ => _.ticker === ticker)?.candles?.[0]
  }
    ...
    return allInstants.map(instant => ({
      ...
      value: sumBy(tickers, ticker => {
        ...
        if (firstCandleInfo !== undefined && lastPrice !== undefined) {
          const items = this.portfolioState.items.filter(_ => _.ticker === ticker)
          const candlePrice = candleGroups.find(_ => _.ticker === ticker)?.candles
            ?.find(_ => _.instant.equals(instant))?.close
  ...
  get selectedCandleGroup(): TickerCandleGroup | undefined {
    ...
    if (this.graphTicker) {
      let info = this.periodInfo.find(_ => _.period === this.period)
      if (info) {
        result = info.candleGroups.find(_ => _.ticker === this.graphTicker)
      }
    }
    return result
  }
  ...
  get selectedStockProfits(): StockProfit[] {
    return this.periodInfo.find(_ => _.period === this.period)?.stockProfits ?? []
  }

  get selectedProfitPoints(): ProfitPoint[] {
    let result: ProfitPoint[] = []
    const group = this.selectedCandleGroup
    const allPoints = this.periodInfo.find(_ => _.period === this.period)?.profitPoints
    if (group && allPoints) {
      const instants = group.candles.map(_ => _.instant)
      result = allPoints.filter(point =>
        instants.some(_ => _.equals(point.instant)))
    }
    return result
  }
}
